    ...
    $.get(URLGET1, function(respuesta1,estado) {
        if(estado === "success") {
            ...
            for (const dato1 of datos1) {
                $("#tabla1").append(`<tbody><td>${dato1.hechizo}</td><td>${dato1.uso}</td></tbody>`);
            }
        }
    })
})
    ...
    $.get(URLGET2, function(respuesta2,estado) {
        if(estado === "success") {
            ...
            for (const dato2 of datos2) {
                $("#tabla2").append(`<tbody><td>${dato2.libro}</td><td>${dato2.titulo_original}</td><td>${dato2.fecha_de_lanzamiento}</td><td>${dato2.descripcion}</td></tbody>`);
            }
        }
    })
})
    ...
    $.get(URLGET2, function(respuesta2,estado) {
        if(estado === "success") {
            ...
            for (const dato2 of busqueda2) {
                $("#tabla2").append(`<tbody><td>${dato2.id}</td><td>${dato2.titulo_original}</td><td>${dato2.fecha_de_lanzamiento}</td><td>${dato2.descripcion}</td><td class="hidden">${dato2.id}</td></tbody>`);
            }
        }
    })
})
